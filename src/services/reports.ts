...
export async function triggerScheduledReport(id: string): Promise<{ report: ReportItem; schedule: ScheduledReport } | undefined> {
  await new Promise((resolve) => setTimeout(resolve, 600));
  const schedule = mockScheduled.find((s) => s.id === id);
  if (!schedule) return undefined;

  schedule.lastGenerated = 'Just now';

  const randomHash = '0x' + Array.from({ length: 48 }, () => Math.floor(Math.random() * 16).toString(16)).join('');
  const newReport: ReportItem = {
    id: `REP-2026-${Math.floor(1000 + Math.random() * 9000)}`,
    name: `${schedule.title} (Auto-Triggered)`,
    category: schedule.category,
    generatedBy: 'Automated Job Runner (Cron)',
    generatedAt: 'Just now',
    period: schedule.frequency === 'Daily' ? 'Last 24 Hours' : schedule.frequency === 'Weekly' ? 'Last 7 Days' : 'Last 30 Days',
    format: schedule.format,
    size: `${(Math.random() * 3 + 1).toFixed(1)} MB`,
    status: 'Completed',
    cryptographicHash: randomHash,
    blockRange: '#2,345,670 - #2,345,678',
    recordsCount: Math.floor(400 + Math.random() * 1800),
    description: `Automated recurring report generated by scheduled job ${schedule.id} and dispatched to: ${schedule.recipients.join(', ')}.`,
    summaryMetrics: [
      { label: 'Schedule ID', value: schedule.id },
      { label: 'Dispatched To', value: `${schedule.recipients.length} recipients` },
      { label: 'Integrity Check', value: '100% Sealed' }
    ]
  };

  mockReports = [newReport, ...mockReports];
  return { report: newReport, schedule };
}
