...
export default function SmartContracts() {
  ...
  const loadContractsData = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        setTotalFilteredCount(
          contractsRes.totalFilteredCount
        );

        setTotalTotalCount(
          contractsRes.totalTotalCount
        );
        ...
        setError(
          'Unable to load contracts from database. Showing fallback data.'
        );
      } finally {
        setIsLoading(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage, monitor, and verify smart contracts
            powering the BEL Trust Platform.
          </p>
        </div>
      ...
      {isLoading && !isRefreshing ? (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center text-slate-500">
          ...
          <p className="font-semibold text-slate-700">
            Loading smart contracts from database...
          </p>

          <p className="text-xs text-slate-400 mt-1">
            Retrieving deployed contracts, ABI specifications,
            and security audits
          </p>
        </div>
      ) : (
