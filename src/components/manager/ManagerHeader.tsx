...
export default function ManagerHeader() {
  ...
  const [userName, setUserName] = useState('BEL Manager');
  const [userRole, setUserRole] = useState('Manager');
  const [userInitials, setUserInitials] = useState('BM');
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const name = raw.name || `${raw.firstName || ''} ${raw.lastName || ''}`.trim() || 'BEL Manager';
        const role = raw.role?.name || raw.role || 'Manager';
        setUserName(name);
        setUserRole(role);
        setUserInitials(
          name
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 relative z-20">
      ...
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 bg-violet-100 text-violet-700 rounded-lg flex items-center justify-center">
          <ShieldCheck className="w-5 h-5" />
        </div>
        <div>
          <p className="text-sm font-bold text-slate-900 leading-tight">Manager Control Center</p>
          <p className="text-[11px] font-semibold text-violet-600 leading-tight">Operations · Approvals · Compliance</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3" ref={menuRef}>
        ...
        >
          <div className="w-8 h-8 bg-gradient-to-tr from-violet-600 to-fuchsia-600 rounded-lg flex items-center justify-center text-white text-xs font-bold">
            {userInitials}
          </div>
          <div className="text-left hidden sm:block">
            <p className="text-xs font-bold text-slate-800 leading-tight">{userName}</p>
            <p className="text-[11px] font-semibold text-violet-600 leading-tight">{userRole}</p>
          </div>
          ...
          <div className="absolute right-6 top-14 mt-2 w-60 bg-white border border-slate-200 rounded-2xl shadow-xl py-2 z-50">
            <div className="px-4 py-2 border-b border-slate-100">
              ...
              <p className="text-[11px] text-slate-500 font-mono truncate">
                {(() => {
                  try {
                    return JSON.parse(localStorage.getItem('bel_user') || localStorage.getItem('user') || '{}').did || 'did:bel:sov:manager01';
                  } catch {
                    return 'did:bel:sov:manager01';
                  }
                })()}
              </p>
              <span className="inline-flex items-center gap-1 mt-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 border border-violet-200">
                {userRole}
              </span>
            </div>
